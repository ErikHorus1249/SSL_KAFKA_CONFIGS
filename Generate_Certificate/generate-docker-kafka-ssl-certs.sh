...
# to amend by erikhorus

# set password for keystore and truststore , Default : aisoft
PASSWORD="aisoft"
...
cd certs
keytool -keystore $SERVER_KEYSTORE_JKS -alias localhost -validity $EXPIRATION_DATE -genkey -storepass $PASSWORD -keypass $PASSWORD \
  -dname "CN=kafka.docker.ssl, OU=None, O=None, L=Hanoi, S=Hanoi, C=VN"
openssl req -new -x509 -keyout ca-key -out ca-cert -days 730 -passout pass:$PASSWORD \
   -subj "/C=VN/S=Hanoi/L=Hanoi/O=None/OU=None/CN=kafka.docker.ssl"
